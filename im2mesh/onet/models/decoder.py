...
from im2mesh.vn_layers import VNStdFeature
...
class VNDecoderCBatchNorm(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
        ...
        self.std_feature = VNStdFeature(c_dim//3, dim=4, normalize_frame=False, negative_slope=0.0)
    ...
    def forward(self, p, z, c, **kwargs):
        ...
        p_norm = p.norm(dim=1, keepdim=True)    # B*1*T
        ### c: B*F
        c = c.reshape(batch_size, -1, 3).transpose(1, 2)    #B*3(D)*F
